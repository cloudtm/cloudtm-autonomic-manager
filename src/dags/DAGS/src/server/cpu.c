...
static double get_cpu_service_demand(SERVER_lp_state_type *pointer, event_content_type *event_content) {
	...
	if (event_content->applicative_content.op_type == CPU_LOCAL_TX_GET)
		return pointer->configuration.local_tx_get_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_LOCAL_TX_PUT)
		return pointer->configuration.local_tx_put_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_REMOTE_TX_PUT)
	...
	else if (event_content->applicative_content.op_type == CPU_SEND_REMOTE_TX_GET)
		return pointer->configuration.tx_send_remote_tx_get_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_LOCAL_TX_GET_FROM_REMOTE)
		return pointer->configuration.local_tx_get_from_remote_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_REMOTE_TX_GET_RETURN)
		return pointer->configuration.remote_tx_get_return_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_UPDATE)
		return pointer->configuration.update_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_LOCAL_PREPARE_SUCCESSED)
		return pointer->configuration.local_prepare_successed_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_LOCAL_PREPARE_FAILED)
		return pointer->configuration.local_prepare_failed_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_DISTRIBUTED_FINAL_TX_COMMIT)
		return pointer->configuration.distributed_final_tx_commit_cpu_service_demand;
	else if (event_content->applicative_content.op_type == CPU_LOCAL_TX_FINAL_COMMIT)
