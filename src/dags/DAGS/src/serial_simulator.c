...
int deliver_next_event() {
	if (first_event) {
		...
		ProcessEvent(event_to_deliver->receiver, event_to_deliver->timestamp, event_to_deliver->event_type, event_to_deliver->event_content,(unsigned int) sizeof(event_to_deliver->event_content) , object_states[event_to_deliver->receiver]);
...
int main(int argc, char* argv[]) {
	int i;
	char *parameters[4] = {"conf-file", "simulation.conf"};
	numerical_init();
	// send INIT event to all simulation objects
	for(i=0;i<NUM_OBJECTS;i++){
		cur_me=i;
		ProcessEvent(i, 0., 0, (void*) &parameters, 2, object_states[i]);
	}
