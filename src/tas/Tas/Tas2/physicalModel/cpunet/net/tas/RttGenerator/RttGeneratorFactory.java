 ...
 *         Date: 03/12/12
 */
public class RttGeneratorFactory {

   public static RttGenerator buildRttGenerator(CubistConfig config) throws Tas2Exception {
      switch (config.getSolvingMethod()) {
         case PURE_RECURSION:
            return new RecursiveRttGenerator(config.getRecursiveRttGeneratorConfig());
         case ITERATIVE:
            return new IterativeRttGenerator(config.getIterativeRttGeneratorConfig());
         case BINARY_SEARCH:
            return new BinaryRttGenerator(config.getBinaryRttGeneratorConfig());
         default:
