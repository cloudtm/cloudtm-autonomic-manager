...
public class TpccPopulation {
   ...
   private boolean populateOnOneCache = false;

   private boolean enablePopulate = true;


   public TpccPopulation(CacheWrapper wrapper, int numWarehouses, int slaveIndex, int numSlaves, long cLastMask, long olIdMask, long cIdMask, boolean populateOnOneCache, boolean enablePopulate) {
      ...
      this.populateOnOneCache = populateOnOneCache;
      ...
      boolean done = false;
       ...
       if(enablePopulate){
           ...
           if(!populateOnOneCache){

               done = wrapper.populateTpcc(this.numWarehouses, this.slaveIndex, this.numSlaves, this.cLastMask, this.olIdMask, this.cIdMask);
           }
           else{

               if(this.slaveIndex == 0){
                   done = wrapper.populateTpcc(this.numWarehouses, this.slaveIndex, 1, this.cLastMask, this.olIdMask, this.cIdMask);
               }
               else{
                   done = true;
               }
           ...
           if(done == false){
               ...
               log.warn("Unable to populate Tpcc via stored procedure on the server.");
