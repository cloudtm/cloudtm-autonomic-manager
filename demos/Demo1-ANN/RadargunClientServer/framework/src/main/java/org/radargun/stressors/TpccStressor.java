...
public class TpccStressor extends AbstractCacheWrapperStressor {
   ...
   private double backoffTime = 0.0D;

   private double thinkTime = 0.0D;
   ...
   private List<Stressor> executeOperations() throws Exception {
      ...
      for (int threadIndex = 0; threadIndex < numOfThreads; threadIndex++) {
         Stressor stressor = new Stressor(threadIndex, this.nodeIndex, this.perThreadSimulTime, this.arrivalRate, this.backoffTime, this.thinkTime, this.paymentWeight, this.orderStatusWeight);
      ...
      for (Stressor stressor : stressors) {
       ...
       File statsFileP = new File("txProfilePayment.txt");
       File statsFileN = new File("txProfileNewOrder.txt");

       PrintWriter outP;
       PrintWriter outN;

       outP = new PrintWriter(statsFileP);
       outN = new PrintWriter(statsFileN);

       if(outP!= null){

           Set<String> keys = new HashSet<String>();

           keys.addAll(PaymentTransaction.getMap.keySet());


           //System.out.println("OutP size keyset = "+keys.size());
           keys.addAll(PaymentTransaction.putMap.keySet());

           for(String key: keys){

               AtomicInteger p = PaymentTransaction.putMap.get(key);
               AtomicInteger g = PaymentTransaction.getMap.get(key);

               String toPrint = ""+key+";"+(p==null?0:p.get())+";"+(g==null?0:g.get());
               outP.println(toPrint);
       ...
       if(outN != null){
           ...
           keys.addAll(NewOrderTransaction.getMap.keySet());
           //System.out.println("OutN size keyset = "+keys.size());
           keys.addAll(NewOrderTransaction.putMap.keySet());

           for(String key: keys){

               AtomicInteger p = NewOrderTransaction.putMap.get(key);
               AtomicInteger g = NewOrderTransaction.getMap.get(key);

               String toPrint = ""+key+";"+(p==null?0:p.get())+";"+(g==null?0:g.get());
               outN.println(toPrint);
               //System.out.println(toPrint);
   ...
   private class Stressor extends Thread {
      ...
      private Random randomThinkTime;

      private double backoff_mean_time;
      private double think_mean_time;


      public Stressor(int threadIndex, int nodeIndex, long simulTime, double arrivalRate, double backoffMeanTime, double thinkMeanTime, double paymentWeight, double orderStatusWeight) {
         ...
         this.backoff_mean_time = backoffMeanTime;
         this.think_mean_time = thinkMeanTime;

         this.paymentWeight = paymentWeight;
         this.orderStatusWeight = orderStatusWeight;

         this.randomThinkTime = new Random(System.currentTimeMillis());

      }

      private double exp(double mean_time) {

         return -Math.log(1.0 - randomThinkTime.nextDouble()) * mean_time;
      }
      ...
      public void run() {
         ...
         TpccTransaction lastAbortedTransaction = null;
         ...
         CacheWrapper transactionalCacheWrapper = cacheWrapper.transactify();
         ...
         while (delta < (this.simulTime * 1000000000L)) {
            ...
            if (arrivalRate != 0.0) {  //Open system
               ...
               if(lastAbortedTransaction != null){
                   transaction = lastAbortedTransaction;

                   thinkTime = (long) exp(backoff_mean_time);

               }
               else{
                   transaction = terminal.choiceTransaction();

                   thinkTime = (long) exp(think_mean_time);
               }
            ...
            transactionalCacheWrapper.startTransaction();

            try {
               transaction.executeTransaction(transactionalCacheWrapper);
            ...
            try {
               ...
               transactionalCacheWrapper.endTransaction(successful);

               if (!successful) {

                  lastAbortedTransaction = transaction;
